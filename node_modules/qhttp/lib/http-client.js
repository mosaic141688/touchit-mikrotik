...
// refactor: chunks = null, assign first chunk, only allocate array on second chunk
            var chunks = null;
            res.on('data', function(chunk) {
                if (!chunks) chunks = chunk;
                else if (!Array.isArray(chunks)) chunks = new Array(chunks, chunk);
            ...
            res.on('end', function() {
                var body = res.body = !Array.isArray(chunks) ? chunks : chunks[0] instanceof Buffer ? Buffer.concat(chunks) : chunks.join('');
...
// TODO: maybe default encoding to 'utf8', instead of `null` (binary)
// TODO: decode json if encoding: 'json' or json: true
            });
        });
...
// TODO: refactor:
//        var options = merge({}, this._httpOptions, (typeof uri === 'object') ? uri : this._parseUrl(uri));
//        if (options.url) merge(options, this._parseUrl(options.url));
//        return options;
...
function merge( target /* ,VARARGS */ ) {
    for (var i=1; i<arguments.length; i++) {
        var src = arguments[i];
        for (var k in src) {
            if (isHash(src[k])) target[k] = isHash(target[k]) ? merge(target[k], src[k]) : merge({}, src[k]);
            else target[k] = src[k];
        }
        return target;
    }
